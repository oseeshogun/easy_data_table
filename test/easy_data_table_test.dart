...
void main() {
  testWidgets('EasyDataTable should render with the correct number of rows and columns', (WidgetTester tester) async {
      ...
      MaterialApp(
        home: Scaffold(
          body: EasyDataTable<Person>(
            columns: [
              EasyColumn(headerText: 'Name', cellText: (item, index) => item.name),
              EasyColumn(headerText: 'Age', cellText: (item, index) => item.age.toString()),
            ],
            rows: const [
    ...
    tester.runAsync(() async {
      ...
      expect(find.byType(DataCell), findsNWidgets(4)); // Assuming two columns in each row
    });
  });

  testWidgets('Selecting a row should trigger the onSelectChanged callback', (WidgetTester tester) async {
